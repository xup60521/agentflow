...
test('OpenCode plugin coalesces progressive Unicode text regardless of event order', async () => {
  ...
  try {
    ...
    await plugin.event({ event: { type: 'message.part.updated', properties: { part: { type: 'text', messageID: 'assistant-part', sessionID: 's1', text: 'assistant text' } } } })
    await plugin.event({ event: { type: 'message.updated', properties: { info: { id: 'a1', sessionID: 's1', role: 'assistant' } } } })
